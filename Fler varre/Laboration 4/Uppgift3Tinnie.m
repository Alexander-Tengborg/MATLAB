%% Laboration 4.3.1
clc
f = @(x,y)(x.*y.^2.*exp(6-4*x.^2+x.*y-3*y.^2));
syms x y
Df=@(x)[-x(2).^2.*(8.*x(1).^2-x(2).*x(1)-1).*exp(6-4*x(1).^2+x(1).*x(2)-3*x(2).^2)
        -x(1).*x(2).*(6*x(2).^2-x(1).*x(2)-2).*exp(6-4*x(1).^2+x(1).*x(2)-3*x(2).^2)];
    
kmax = 5;          % hur många steg man är villig att gå
tol = 0.5e-5;       % stoppvillkoret (den steglängd som anses oväsentlig)
aktuelltx = [-0.41; -0.65]; % startpunkt
for k=1:kmax
