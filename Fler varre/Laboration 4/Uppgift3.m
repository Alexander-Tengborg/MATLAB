%% Laboration 4.3.1
clc
f = @(x,y)(x.^2.*y.*exp(5-3*x.^2-x.*y-y.^2));
syms x y
Df=@(x)[-x(2).*x(1).*(6*x(1).^2+x(2).*x(1)-2).*exp(5-3*x(1).^2-x(1).*x(2)-x(2).^2)
        -x(1).^2.*(2*x(2).^2+x(1).*x(2)-1).*exp(5-3*x(1).^2-x(1).*x(2)-x(2).^2)];
    
kmax = 5;          % hur många steg man är villig att gå
tol = 0.5e-5;       % stoppvillkoret (den steglängd som anses oväsentlig)
aktuelltx = [0.61; -0.91]; % startpunkt
for k=1:kmax
